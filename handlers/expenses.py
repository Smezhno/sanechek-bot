...
async def cost_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not args:
        ...
        await update.message.reply_text("Какая сумма?")
    ...
    if parsed["amount"] is not None:
        ...
        if parsed["description"]:
            ...
            await update.message.reply_text("Опиши расход")
...
def _parse_cost_command(text: str) -> dict:
    ...
    if amount_match:
        ...
        try:
            ...
            if amount > settings.max_expense_amount:
                result["error"] = "Сумма слишком большая. Проверь, нет ли ошибки"
                return result
            ...
            result["error"] = "Не понял сумму. Введи число"
...
async def receive_cost_amount(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if parsed["amount"] is not None:
        ...
        error = parsed["error"] or "Не понял сумму. Введи число"
